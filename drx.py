...
CONFIG = {
    ...
    "feedback_group_id": -1003948467682, # Apne group ki ID yahan dalein
}
# --- [ EXPIRY CONFIGURATION ] ---
# Format: datetime(Year, Month, Day, Hour, Minute, Second)
SCRIPT_EXPIRY = datetime(2030, 5, 25, 12, 30, 30) 
...
def handle_bgmi_fixed(message):
    ...
    # --- [ 3. TRIGGER ATTACK ] ---
    attack_entry = {"ip": ip, "port": port, "duration": int(duration), "user": user_id, "end_time": time.time() + int(duration)}
    ...
    for _ in range(5):
        ...
        for i in range(1, CONFIG['max_slots'] + 1):
            if i <= used:
                ...
                rem = max(0, int(att.get('end_time', time.time() + att.get('duration', 60)) - time.time()))
